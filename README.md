...
Configure the Network & Start Server: * For a connection strictly on the same computer, leave the IP as 127.0.0.1.

For a connection from a local PC on your network, the user should put the IP as 0.0.0.0 and open the ports if necessary. This binds the server to all network interfaces.

Note that when you do this, the actual server IP that clients must connect to will be the local PC's IP address (e.g., 192.168.1.50).

Click 🔌 GO ONLINE. The status will change to RUNNING.

Connect Clients: You can now connect your SCADA client, HMI, or the companion Client Dashboard to this server.
...
Stop the Server: Click 🔴 GO OFFLINE to gracefully shut down the async loop and disconnect clients.
